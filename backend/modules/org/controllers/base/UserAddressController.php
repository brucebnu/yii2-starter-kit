...
*/
class UserAddressController extends \backend\controllers\Controller
{
...
public function actionView($user_address_id)
{
    ...
    'model' => $this->findModel($user_address_id),
    ]);
}
...
* @return mixed
*/
public function actionDelete($user_address_id)
{
    exit('sorry :(');
    try {
    $this->findModel($user_address_id)->delete();
    } catch (\Exception $e) {
    $msg = (isset($e->errorInfo[2]))?$e->errorInfo[2]:$e->getMessage();
    ...
    $isPivot = strstr('$user_address_id',',');
    if ($isPivot == true) {
    return $this->redirect(Url::previous());
...
*/
protected function findModel($user_address_id)
{
        if (($model = UserAddress::findOne($user_address_id)) !== null) {
    return $model;
    } else {
